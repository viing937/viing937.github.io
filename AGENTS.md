...
Install dependencies with `bundle install`. Run locally with `bundle exec jekyll serve`. Validate changes with `bundle exec jekyll build`; this is the main check for broken config, front matter, and Liquid templates. Update `Gemfile.lock` through Bundler, not by hand.
